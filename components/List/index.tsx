...
const List = (props: ListProps) => {
  ...
  const renderListItem = () => {
    ...
    const items = data?.map((item: Option) => {
      ...
      return (
        <Menu.Item key={value} onMouseOver={() => handleOver(value)}>
    ...
    return (
      <>
        ...
        >
          {setAll ? (
            <Menu.SubMenu key="0" title="全部" className={selectMenuAll ? 'sub-menu-selected' : ''}>
              {items}
              {showNoGroup ? <Menu.Item key="noGroup">未分组</Menu.Item> : null}
            </Menu.SubMenu>
          ) : (
            items
          )}
  ...
  return (
    <div style={style} className={className + ' menu-select-list'} onMouseOut={handleClean}>
      {renderListItem()}
    </div>
  );
};
